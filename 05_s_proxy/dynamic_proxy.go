...
func genernate(file string) (string, error) {
...
func New{{ .ProxyStuctName }}Proxy(child *{{ .ProxyStructName }}) *{{  .ProxyStructName }}Proxy {
	return &{{ .ProxyStuctName }}Proxy{child: child}
}

{{ range .Methods }}
func (p *{{$.ProxyStuctName}}Proxy) {{ .Name }} ({{ .Params }}) ({{ .Results }}){
	// before 这里会有一些统计逻辑
	start :=time.Now()
